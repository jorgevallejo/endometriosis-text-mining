...
# dummy_dataframe <- data.frame(PMID = c(123, 234),
#                               GeneID = c("123", "123;234"),
#                               stringsAsFactors = FALSE
# )
# 
# gene2pubtator2 <- dummy_dataframe

gene2pubtator_dups <- data.frame(PMID = integer(),
                                 GeneID = character(),
                                 stringsAsFactors = FALSE)

for(i in 1:nrow(gene2pubtator)) {
  if (grepl(";", gene2pubtator$GeneID[i]) == TRUE) {
    # Generate deduplicated row
    x <- gene2pubtator$PMID[i];
    y <- sub(pattern = ";[0-9]+",
             replacement = "",
             gene2pubtator$GeneID[i]);
    z <- data.frame(PMID = x,
                    GeneID = y,
                    stringsAsFactors = FALSE);
    gene2pubtator <- rbind(gene2pubtator, z);
    # Delete duplicated GeneID
    gene2pubtator$GeneID[i] <- sub(pattern = "^[0-9]+;",
        replacement = "",
        gene2pubtator$GeneID[i])
  }
}
