...
Respecto a las desviaciones; varias de las tareas previstas para esta fase están sin terminar o incluso sin empezar, mientras que como se explica en el apartado anterior hay tareas previstas para la fase siguiente que han sido comenzadas antes de tiempo.

Las siguientes tareas de esta fase están **sin finalizar**:

- Filtrado estadístico.  
- Definir esquema de aplicación web.  
- Página de visualización de abstracts
 
Las siguientes tareas se han **comenzado antes de lo previsto**:

- Incorporar los procesos de minería y análisis de texto a la aplicación web.
- Subir aplicación a server de Shiny.

Analizando las **causas** de las desviaciones, creo que hay dos motivos para las mismas. Por una parte, soy lento escribiendo código y eso ha hecho que subestime el tiempo que debía dedicar a algunas de las tareas. Y por otra parte, cuando me he retrasado en una tarea no he sabido abandonarla a tiempo, lo que ha resultado en dejar a medias no sólo la tarea en la que originalmente tenía problemas sino también la tarea siguiente. Por ejemplo, al ver que no conseguía completar el "filtro estadístico" en el tiempo asignado, podría haber abandonado esa tarea y pasar a la de "definir el esquema de la aplicación web"; ya que esta segunda es independiente de la primera, más sencilla, y potencialmente más importante para completar el proyecto.

\newpage

## Acciones de mitigación

Las tareas "Definir esquema de aplicación web" y "Página de visualización de abstracts" las atrasaré a la **Fase 2 de desarrollo del trabajo**. He reorganizado el planning para asignarles tiempo. Básicamente reduciendo los 16 días de "Pruebas de uso y corrección de errores" a sólo 8 días.

```{r reorganinzacion plan trabajo}
tareas <- c("_Fase 2 de desarrollo del trabajo_",
            "Definir esquema de aplicación web",
            "Página de visualización de abstracts",
            "Caracterización de la lista de genes",
            "Visualización de resultados",
            "Incorporar los procesos de minería",
            " y análisis de texto a la aplicación web",
            "Subir aplicación a server de Shiny",
            "Pruebas de uso de la aplicación",
            " y corrección de errores",
            "Documentar en la memoria")

fecha_inicio <- c("_20/04/21_",
                  "20/04/21",
                  "22/04/21",
                  "27/04/21",
                  "27/04/21",
...
dias <- c("_28_", "1", "5", "5", "5", "", "5", "3", "", "8", "28")

hito <- c("_Entrega de PEC3_", rep("", 10))
...
kable(
  ...
  col.names = c("**Tarea**",
                ...
                "**Hito**"),
  align = 'lcccl',
  caption = 'Nuevo plan de trabajo para la PEC 3.'
)
...
```{r cronograma, fig.cap='Cronograma modificado respecto al cronograma original. Las tareas correspondientes a la PEC 3 están representadas en verde.'}
include_graphics("images/cronograma.png")
...
Los resultados parciales que se adjuntan son un script en R y el código de una aplicación Shiny que implementa las mismas funcionalidades del script:

- Búsqueda en PubMed por palabras clave y rango de fechas, y descarga de resultados.  
- Procesado de los resultados de la búsqueda para generar: un corpus de sumarios de artículos científicos, un listado con las palabras incluidas en los sumarios y sus frecuencias, y un listado con los genes incluidos en los sumarios y sus frecuencias.  
- Visualización del listado de palabras y el listado de genes en forma de tabla y de gráfica de barras, ordenados de mayor a menor frecuencia.

El **código de la aplicación Shiny** se entrega como código impreso en el [apéndice D](#apendiceD), y como fichero de texto `app.R` descargable desde [mi repositorio en GitHub](https://github.com/jorgevallejo/endometriosis-text-mining/tree/master/PEC2/shinyapp).

El **script en R** se entrega como código impreso en el [apéndice E](#apendiceE), y como fichero de texto `PEC2_fase1_script.R` descargable desde [este enlace](https://github.com/jorgevallejo/endometriosis-text-mining/tree/master/PEC2) en mi repositorio de GitHub.
...
El documento original en formato .Rmd que incluye el código completo en lenguaje R usado para generar este informe (fichero `PEC2_fase1_informe.Rmd`), se puede consultar y descargar en el siguiente repositorio de Github:
...
download.file(url = "ftp://ftp.ncbi.nlm.nih.gov/pub/lu/PubTatorCentral/
...
## Apéndice E: Código para minería de textos y gráficas{#apendiceE}
