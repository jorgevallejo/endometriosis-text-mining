...
Package with tool for text mining of PubMed abstracts (text and XML).
...
Las funciones principales que usaremos de este paquete son `get_pubmed_ids()`, que nos permite realizar una búsqueda en PubMed; `fetch_pubmed_data()`, con la que podemos recuperar registros de PubMed tras una búsqueda con la función anterior y `batch_pubmed_download()`, que permite descargar los resultados de una búsqueda y es la más adecuada cuando se trata de un elevado número de registros. Los resultados se pueden descargar en formato XML o TXT en lotes de hasta 5 000 registros. Estos resultados en formato texto conforman los datos sobre los que usaremos las funciones del paquete _pubmed.mineR_.
